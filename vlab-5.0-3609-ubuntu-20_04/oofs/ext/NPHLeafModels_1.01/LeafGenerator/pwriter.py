...
def Input(pdict, wd, wid):
	...
	lines = [
		...
		"float INITSTATE[2*NSAMP-1]={"+",".join([str(int(x)) for x in b if not np.isnan(x)])+"};",
		...
		"#define LASSYMETRY {LASSYMETRY}".format(**pdict), #Potentially good one to alter
		"#define RASSYMETRY {RASSYMETRY}".format(**pdict),
		"#define DIVTHRESH {DIVTHRESH}".format(**pdict),
		"#define CP_DIST {CP_DIST}".format(**pdict), 
		"#define FILLNONAPICALINTERVALS {FILLNONAPICALINTERVALS}".format(**pdict),
		"{DEFINE_TERMINAL_LEAFLET_BASE}".format(**pdict), #for 12f and 12de
		"{BOUNDARY_MORPHOGEN}".format(**pdict), #CHANGE TO 3 for 12f and de
		"",
		...
		"#define EXTENDABLE_CONTEXT1 {EXTENDABLE_CONTEXT1}".format(**pdict),
		...
		"#define EXTENDABLE_CONTEXT2 {EXTENDABLE_CONTEXT2}".format(**pdict),
		...
		"#define EXTENDABLE_CONTEXT3 {EXTENDABLE_CONTEXT3}".format(**pdict),
		...
		"#define EXTENDABLE_CONTEXT4 {EXTENDABLE_CONTEXT4}".format(**pdict),
