    ...
    get_treeness <- function(summary){
      ...
      for (dataset in summary$dataset) {
        ...
        if (class(tree) == "phylo") {
          summary[summary$dataset == dataset, "ntips"] <- length(tree$tip.label)
          ...
          for (i in seq_along(tree)) {
            ntp_list <- c(ntp_list, length(tree[[i]]$tip.label))
          ...
          summary[summary$dataset == dataset, "ntips"] <- mean(ntp_list)
