...
wd = "/home/m/malone/vlab-5.0-ubuntu-20.04/oofs/ext/NPHLeafModels_1.01/LeafGenerator"
...
def classify_test():
    ...
    #actual data
    labeled_data = pd.read_csv("/home/m/malone/leaf_storage/accuracy_test_500/trainingdata_morph.csv")

    unlabeled_data = pd.read_csv("/home/m/malone/leaf_storage/accuracy_test_200/shape_report.csv")
    #unlabeled_data["shape"] = None
    #print(unlabeled_data)

    labeled_features = labeled_data[["no.extrema","minmax_dist_avg","minmax_angle_avg","minima_samerow_dist_avg","refmax_dist_avg"]].values
    ...
    unlabeled_features = unlabeled_data[["no.extrema","minmax_dist_avg","minmax_angle_avg","minima_samerow_dist_avg","refmax_dist_avg"]].values
    ...
    unlabeled_data.to_csv("/home/m/malone/leaf_storage/accuracy_test_200/prediction.csv", index=False)
...
def classify_batch(wd, wd1):
    ...
    for leafdirectory in os.listdir(wd1 + "/leaves_full_28-8-23_MUT1"):
        ...
        leafdirectory_path = os.path.join(wd1 + "/leaves_full_28-8-23_MUT123", leafdirectory)
...
def classify_test_contour():
    ...
    labeled_data = pd.read_csv("/home/m/malone/leaf_storage/accuracy_test_500/trainingdata_contour.csv", header=None)

    unlabeled_data = pd.read_csv("/home/m/malone/leaf_storage/accuracy_test_200/refdist_report.csv", header=None)
    # #unlabeled_data["shape"] = None
    # print(unlabeled_data)
    ...
    unlabeled_data.to_csv("/home/m/malone/leag_storage/accuracy_test_200/prediction.csv", index=False)
...
def classify_batch_contour(wd, wd1):
    ...
    for leafdirectory in os.listdir(wd1 + "/leaves_full_28-8-23_MUT1"):
        print(f"Current = {leafdirectory}")
        leafdirectory_path = os.path.join(wd1 + "/leaves_full_28-8-23_MUT1", leafdirectory)
        for walkdirectory in os.listdir(leafdirectory_path):
            walkdirectory_path = os.path.join(leafdirectory_path, walkdirectory)
            for file in os.listdir(walkdirectory_path):
    ...
    filled_dfs = [df.reindex(columns=widest_df.columns, fill_value=np.nan) for df in dfs]

    unlabeled_data = pd.concat(filled_dfs, axis=0, ignore_index=True)
    unlabeled_data.to_csv(wd1 + "/unlabeled_data.csv", index=False, header=False)
    ...
    prediction = pd.DataFrame({"leafid":unlabeled_data.iloc[:,0], "walkid":unlabeled_data.iloc[:,1], "predicted":predicted_labels})
